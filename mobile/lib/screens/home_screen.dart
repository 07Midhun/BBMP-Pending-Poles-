...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _onRegionChanged(String? newRegion) async {
    ...
    try {
      ...
      final zones = await ApiService.fetchZones(newRegion);
      ...
      setState(() {
        ...
        _loadingZones = false;
        _zoneLoadError =
            zones.isEmpty ? 'No zones returned for $newRegion' : null;
      });
    } catch (_) {
      if (!mounted || generation != _selectionGeneration) return;
  ...
  Future<void> _onPoleOldLampChanged(String? newOldLamp) async {
    ...
    try {
      final types = await ApiService.fetchLampTypes(newOldLamp);
  ...
  Future<void> _uploadPoleImage({
    ...
    try {
      ...
      final uri = Uri.parse(
        '$baseUrl/api/v1/poles/${Uri.encodeComponent(pole.poleNumber)}/images/$slot',
      );
                          ...
                          _selectedStartingPole ??
                              (_loadingPoles
                                  ...
                                  : (_filteredPoles.isEmpty
                                      ...
                                      : 'Select Starting Pole to Order Nearest → Farthest')),
