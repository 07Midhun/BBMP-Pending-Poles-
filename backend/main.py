...
def get_lamp_types(
    pole_old_lamp: str = Query(
        ...,
        description="Selected Pole With Old Lamp classification",
    )
):
    """
    Return the exact Lamp Type choices allowed by the application.

    The production classifier stores LED profiles as comma-separated values,
    while the UI exposes the four supported LED combinations. Empty poles
    are represented by '-' in live records; the UI also accepts Blank.
    """
    selected = pole_old_lamp.strip().casefold()
    ...
    if selected == "non-led":
        ...
        all_types = {
            str(p.get("lamp_type") or "").strip()
            for p in _get_live_pending_records()
            if str(p.get("lamp_type") or "").strip()
        }

        non_led_types = sorted(
            t
            for t in all_types
...
def _normalize_filter_lamp_type(value: Optional[str]) -> Optional[str]:
    """Normalize UI lamp-type aliases to the representation used by records."""
    if value is None:
        return None
